...
export default function MemberAddCollectionPage() {
  ...
  const handleSubmit = async (formData: any) => {
    ...
    try {
      ...
      const code = `RPB-${Date.now().toString().slice(-6)}`;
      const slug = `${formData.title
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/(^-|-$)+/g, '')}-${Date.now().toString().slice(-4)}`;

      // 2. Insert Koleksi dengan created_by = user.id
    ...
    <div className="space-y-6">
      <div className="border-b border-white/10 pb-4">
        <h1 className="text-2xl font-serif text-[#D4AF37] font-bold">
          Unggah Koleksi Barang Antik
        </h1>
        <p className="text-xs text-[#F5F2EB]/60 mt-1">
          Lengkapi data spesifikasi fisik dan foto barang antik Anda.
        </p>
      </div>
